...
struct ContentView: View {
  ...
  // Bundle.main.path(forResource: "install_tools", ofType: "sh"): Looks for a shell script named "install_tools.sh" in the app bundle
  // ?? "/bin/bash": If the script isn't found (nil), defaults to the bash shell
  // The nil-coalescing operator (??) provides a fallback value
  
  private func runCommand(for tool: Tool) {
    let process = Process()
    process.launchPath = Bundle.main.path(forResource: "install_tools", ofType: "sh") ?? "/bin/bash"
    // Pass tool name as an argument or use script logic to handle "all"
    process.arguments = [tool.name] 
    process.launch()
    process.waitUntilExit()
    if process.terminationStatus == 0 {
        alertMessage = "Installed \(tool.name) successfully."
    } else {
        alertMessage = "Failed to install \(tool.name)."
    }
    showingAlert = true
}
